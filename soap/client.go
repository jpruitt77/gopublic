...
type XMLMarshaller interface {
	Marshal(v interface{}, version string) ([]byte, error)
	Unmarshal(xml []byte, v interface{}, version string) error
}

type defaultMarshaller struct {
}

func (dm *defaultMarshaller) Marshal(v interface{}, version string) (xmlBytes []byte, err error) {
	xmlBytes, err = xml.MarshalIndent(v, "", "	")

	return adjustNameSpace(xmlBytes, version), err
}

func (dm *defaultMarshaller) Unmarshal(xmlBytes []byte, v interface{}, version string) error {
	xmlBytes = adjustNameSpace(xmlBytes, version)
...
func adjustNameSpace(xmlBytes []byte, version string) []byte {
	if version == SoapVersion12 {
		l("changing namespace...")
		tmp := string(xmlBytes)
		tmp = strings.Replace(tmp, NamespaceSoap11, NamespaceSoap12, -1)
		tmp = strings.ReplaceAll(tmp, "soap:", "soap12:")
		tmp = strings.Replace(tmp, "soap=", "soap12=", -1)
		xmlBytes = []byte(tmp)
	}
	return xmlBytes
}
